...
if (!OUTPUT_DIR) {
  console.error(
    "Usage: node convert-sessions.js <output-directory> [--session <session-id>]",
  );
  process.exit(1);
}
...
function projectName(cwd, dirName) {
  ...
  if (cwd) {
    const segments = cwd.split("/").filter(function (s) {
      return s.length > 0;
    });
...
function convertSubagents(dirPath, sessionDirName, project) {
  ...
  for (let af = 0; af < agentFiles.length; af++) {
    ...
    const agentShortId = agentFiles[af]
      .replace(".jsonl", "")
      .replace("agent-", "")
      .slice(0, 12);
    ...
    const outFile = path.join(
      outDir,
      agentDate + "-" + parentSlug + "-sub-" + agentShortId + ".md",
    );
    ...
    fs.writeFileSync(
      outFile,
      buildSubagentMarkdown(agentData, project, parentSlug),
      "utf8",
    );
    result.subagents++;
  }
...
function main() {
  ...
  if (SESSION_ID) {
    ...
    for (let d = 0; d < dirs.length; d++) {
      ...
      const peekCwd =
        files.length > 0 ? peekFields(path.join(dirPath, files[0])).cwd : "";
