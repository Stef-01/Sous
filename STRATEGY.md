...
| Apr 2026 | 20-phase intuition + beauty build (Stage 0.5)                                                    | Raised the floor for readability, tap targets, and plain language; reworked the Path screen into an ambient, living surface; added habit rails (Tonight, Plan my cook, Repeat cook, Rest day, Confidence dial); opened the network-effect door (Recipe gift, Cook-for-two). Shipped without violating radical simplicity — every new element is quiet, opt-in, and earns its pixel space.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Apr 2026 | Big-hands mode is session-scoped, not a setting                                                  | One accidental edge-tap nudges the user to enable it, disabling it at session end. Preserves "no settings page" while still meeting accessibility needs for users with larger motor targets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Apr 2026 | Rest-day grace: 1 per 7 days, preserves streak                                                   | Streak fragility is the main driver of churn in habit apps. A capped rest-day preserves momentum without breaking the commitment signal — taking two in a row is impossible, so the streak still means something.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Apr 2026 | Kitchen confidence dial shows a tier label, never a number                                       | Numbers invite comparison ("I'm a 62, my friend is a 78"). Tiers (Steady/Bold/Fluent/Master) invite self-identification. Same data, very different emotional load.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Apr 2026 | Household taste blend is a single knob, not a profile                                            | A second-user profile would require onboarding, a settings page, and permanent state — all of which break the architecture. Dampening the primary user's preference vector toward neutral captures 80% of the value with none of the complexity.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| May 2026 | Supabase is the production platform; AI integrations staged last                                 | Consolidate the backend on Supabase (Postgres+RLS, Auth, Storage, Realtime, Edge Functions, pg_cron) rather than Clerk+R2+Upstash, cutting vendor sprawl and getting auth-integrated RLS for free (Clerk/R2/Upstash remain documented per-surface fallbacks). All deterministic, live capability ships before any LLM call goes real: the AI surfaces (food recognition, craving parse, coach, autogen, voice, viral search) stay on their mock providers until the final stage when `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`/search keys + cost guardrails are wired. The product is a complete cooking app without AI; AI is the enhancement layer. See `docs/PRODUCTION-READINESS-PLAN.md`.                                                                                                                                                                                                                                                                                                                                                                                         |
| Jun 2026 | Culinary Therapeutics = a 9th, evidence-graded scorer — not a medical product                    | Operationalize the culinary-therapeutics evidence matrix as a deterministic therapeutic-fit scorer (#9) + a hard-exclusion safety screen over the **existing** catalog — intelligence over inventory (§1.1), not a new "medical recipes" database. Capture lives only in the owl Profile & Settings sheet (rule 3); output rides existing result cards below the hero (rule 13); zero-condition users get byte-identical rankings (the therapeutic weight is 0 unless a condition is active). Anti-overclaiming is enforced in code via the intervention-class taxonomy (recipe-native vs fortified-food vs extract-or-supplement — only the first two can move a recipe's score). Ships `isEducational` behind five founder gates: G1 clinical sign-off, G2 real recipe sourcing (rule 7), G3 lab/EHR inputs, G4 Stanford/CIA branding permission (rule 11), G5 legal/regulatory claim review. "Food-first, not food-only" is structural (escalation subsystem). Strengthens the Engine, Data, and Content moats simultaneously. Full plan: `docs/CULINARY-THERAPEUTICS-PLAN.md`. |
| Jun 2026 | 20-week autonomous moat-deepening plan (AUTO-BUILD track)                                        | Sequenced 20 weeks of fully AI-executable work that deepens a named compounding moat each week, while the founder-gated rails (Auth, Storage, Realtime, Stripe, AI keys, beta cohorts, legal/clinical sign-off) get credentials in parallel. Ordered by the §7 impact/effort matrix and the North Star (cooks/week): Engine×Data context-intelligence first (W1–4), Content depth + voice (W5–8), Behavioral habit-loop depth (W9–12), craft+reliability (W13–16), Network-moat prep + founder-gated integration stubs (W17–20). Per rule 12, every founder-gated integration ships its adapter/env-contract stub so go-live is one config edit; the lean key-less app stays fully functional throughout (§9.3). Full plan: `docs/20-WEEK-AUTONOMOUS-MOAT-PLAN.md`.                                                                                                                                                                                                                                                                                                                |
| Jun 2026 | Home-chef surplus-meal marketplace → PARKED (§12.11)                                             | Off-thesis (Sous is cooking, not someone-else-cooks-for-you) + founder-gated: a two-sided marketplace needs real restaurant partners, kitchen ops, payments, pickup logistics, food-safety liability — none AI-executable. Sibling of 12.4/12.10. The auto-buildable half (eat-out discovery + taste personalization) is already shipped (`/eat-out`); the marketplace half is now the rule-12 prep — a typed `HomeChefBatch` schema + demo fixtures + a `homeChefEnabled`-OFF, flywheel-wired "surplus specials" rail (**built Jun 2026**; renders null until a real partner feed + env flag) — no fake marketplace UI shipped to users (depth-over-speed).                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Jun 2026 | Onboarding v2 + pulse micro-surveys + food-glyph icon system (mockup-driven)                     | 28 founder-committed reference mockups (docs/PLANS, measured in docs/ONBOARDING-SURVEY-DESIGN-KIT.md) define the capture grammar: goal→friction→relatable-statement-swipe→tastes→mirror-summary onboarding arc, plus one-screen "pulse" micro-surveys (deterministically scheduled, ≤1/day, one-tap skip) and a volunteered "Tune my picks" entry in the existing Profile sheet (rule 3). Every captured signal maps to a named engine consumer (effort weight, suppressed tags, cuisine seeds, deficits, nudge cadence) — capture without a consumer is banned, so personalization compounds the Engine and Data moats. UI emojis give way to an inline-SVG line-glyph set for a more crafted surface (pet/achievements exempt). Mirror copy is forward-promise only — fabricated social-proof stats are banned by test. Spec: planning.md §6.2 (D-19…D-23).                                                                                                                                                                                                                      |
| Jul 2026 | Close the 20-week AUTO-BUILD track; next work is credential rehearsal or narrow friction removal | W20 closed the autonomous moat track with the founder-gate punch list and regression record in `docs/20-WEEK-AUTONOMOUS-MOAT-RETRO.md`. The strategic lesson is to avoid another broad feature lane until founder unlocks are exercised: auth + database first, then storage/realtime/payments/AI in that order as needed. If credentials are still absent, the only justified autonomous work is a tightly scoped friction reduction in the core craving -> cook -> win loop, because the North Star remains cooks completed per user per week.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
### 12.11 Home-Chef Surplus Meal Marketplace

- **Inspiration:** Too Good To Go (surplus-food rescue), ghost-kitchen ops, CSA boxes
- **What:** Local / favorite restaurants use the app to produce discounted, home-packed meals at scale — including daily specials built from unsold / over-ordered **surplus ingredients** (waste-reduction angle). The user's existing eat-out taste profile chooses who/what to surface.
- **Why ambitious:** A two-sided marketplace — supply-side onboarding, kitchen ops, food-safety/handling liability, dynamic surplus inventory, payments, pickup/fulfilment logistics, per-batch nutrition. **None of it is AI-executable; all of it is founder-gated** on real restaurant partners + ops.
- **Off-thesis check (rule 8):** The thesis is COOKING (craving→cook habit loop; "cooking is the content"). This is _someone-else-cooks-for-you_ — it competes with the cooking habit rather than compounding it, and does not strengthen the data/content/engine moats directly. Its only moat tie is reusing the eat-out taste profile (a thin link). Sibling of 12.4 (marketplace) and 12.10 (sourcing logistics), both PARKED.
- **Upside:** Genuine differentiator (surplus → waste reduction → cheaper meals); deepens the eat-out flywheel; marketplace/B2B revenue; "when you genuinely can't cook tonight" coverage without leaving the app.
- **Prerequisites:** Proven consumer PMF; a metro with real restaurant partners + a surplus-inventory feed; payments + pickup logistics; food-safety counsel.
- **Auto-build prep shippable now (rule 12, no founder unlock):** (a) the eat-out discovery surface + taste personalization — **DONE** (`/eat-out`); (b) a `HomeChefBatch` data contract (`restaurant, dish, surplusIngredients[], regularPrice, surplusPrice, qtyAvailable, pickupWindow, perBatchNutrition`) as a typed schema + demo fixtures — **DONE** (`src/types/home-chef.ts` Zod schema + `src/data/home-chef/demo-batches.ts`, referencing only real eat-out venues/dishes, rule 7); (c) a `homeChefEnabled`-flagged (OFF) "surplus specials near you" rail wired to the same `recordSignal` flywheel — **DONE** (`src/components/eat-out/surplus-specials-rail.tsx` + `src/lib/home-chef/flag.ts`; renders null unless `NEXT_PUBLIC_SOUS_HOME_CHEF_ENABLED=true` or the `?homechef=1` dev override; taste-sorted, tap → flywheel). A real partner feed is now a one-line `batches` prop swap + the env flag — with **zero fake-marketplace UI shipped to users** meanwhile.
- **Risk:** Logistics/liability nightmare; pulls the product from cooking-confidence toward food delivery; "could become a delivery app" (cf. 12.10's "could become a grocery app").
- **Status:** PARKED — off-thesis + founder-gated. Revisit only after consumer PMF **and** a committed restaurant partner. Until then the only legitimate work is the schema + flag-gated stub above; do NOT ship a user-facing marketplace without real supply (depth-over-speed: no shallow stub to users).
