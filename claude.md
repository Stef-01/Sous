...
3. **No settings pages, with one narrow exception (Stage 2 W9)**: Preferences are still learned from behavior or captured through playful coach interactions wherever possible. The single permitted exception is the **Profile & Settings sheet** opened by tapping the owl mascot in the top-right of the Today header. It is a bottom sheet (NOT a tab — the bottom nav stays Today · Path · Content). It contains the Parent Mode toggle + age band picker, a placeholder for future profile/auth, and the about/disclaimer. Never expose filter panels, preference checklists, multiple settings screens, or push other configuration into this sheet without a written rule-3 amendment.
...
13. **Evaluate the screen, not the element — the simplicity budget (load-bearing rule)**: Before adding ANY visible element to a screen (a chip, card, banner, stat, badge, greeting, helper line), do not ask "is this useful?" — in isolation almost everything is. Ask: **"Does the screen survive this? Is the primary action/hero still first and unburied? Does this restate a signal already shown elsewhere?"** If a signal already exists, **strengthen the existing surface — never add a second copy.** Concretely: cadence is the streak flame and nothing else; **stats live on Path, never on Today**; the hero of every screen (the meal, the recipe, the cook step) gets the top and the air, and contextual nudges are conditional and sit BELOW it. Today's default above the meal is exactly: brand+streak, search, meal — full stop. Adding a Today element is **one-in-one-out**. This is enforced by `src/app/(today)/today/simplicity-budget.test.ts`. This rule was added 2026-06-01 after the Today screen accreted three redundant restatements of cooking cadence ("Day 4 of cooking" + "cook Monday mornings" + "3 cooks this week") that buried the meal — full RCA in `docs/RCA-TODAY-CLUTTER.md`. The failure mode is per-element justification: every chip is a local optimum; their sum is a global mess. Restraint is invisible work and an added widget is legible proof of work, so the bias always tilts toward adding — resist it.
