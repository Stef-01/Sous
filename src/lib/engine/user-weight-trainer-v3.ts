...
function meanDimension(
  ...
  let sum = 0;
  for (const c of cooks) sum += c.engineScoreBreakdown[dim];
...
export function perDimensionDelta(
  ...
  const result = {} as Record<keyof ScoreBreakdown, number>;
