...
describe("trainUserWeightsV3 — signal applied", () => {
  ...
  it("monotonic: stronger signal → bigger shift (within cap)", () => {
    // Both deltas must be BELOW V3_MAX_DELTA (0.1) for the
    // monotonicity to be observable — the cap intentionally
    // clamps any signal at-or-above 0.1 to the same shift.
    const weakHistory: BreakdownCook[] = [
      ...Array.from({ length: 5 }, () =>
        fixtureCook({
          rating: 4,
          engineScoreBreakdown: fixtureBreakdown({ preference: 0.56 }),
        }),
      ),
      ...Array.from({ length: 5 }, () =>
        fixtureCook({
    ...
    // Strong signal: delta 0.09 — still below 0.1 cap, but
    // larger than weak's 0.06.
    const strongHistory: BreakdownCook[] = [
      ...Array.from({ length: 5 }, () =>
        fixtureCook({
          rating: 5,
          engineScoreBreakdown: fixtureBreakdown({ preference: 0.59 }),
        }),
      ),
      ...Array.from({ length: 5 }, () =>
        fixtureCook({
