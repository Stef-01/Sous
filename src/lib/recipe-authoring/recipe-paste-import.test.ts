...
describe("recipe paste-bridge — parse", () => {
  ...
  it("rejects empty / non-JSON / wrong-shape with a friendly reason", () => {
    ...
    const badCuisine = parseRecipeAutogenJson(
      JSON.stringify({ ...STUB_AUTOGEN_RESPONSE, cuisineFamily: "klingon" }),
    );
    expect(badCuisine.ok).toBe(false);
    if (!badCuisine.ok) expect(badCuisine.reason).toMatch(/cuisineFamily/);
  });
