...
export function WhosAtTable() {
  ...
  // Empty-roster CTA — nudge the user to populate /path/household
  // first; the picker only earns its space once there's data.
  if (members.length === 0) {
    return (
      <Link
        href="/path/household"
        className="flex items-center gap-2 rounded-2xl border border-dashed border-neutral-200 bg-white/40 px-4 py-2.5 text-xs text-[var(--nourish-subtext)] transition hover:border-neutral-300 hover:text-[var(--nourish-dark)]"
      >
        <Users size={14} className="text-[var(--nourish-green)]" aria-hidden />
        <span>Add household members for personalised pairings.</span>
      </Link>
    );
  }
