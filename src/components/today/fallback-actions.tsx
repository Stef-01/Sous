...
const chips = [
  { key: "fridge", icon: Refrigerator, label: "Rescue my fridge", handler: "onRescueFridge" as const },
  { key: "game", icon: Gamepad2, label: "Play a game", handler: "onPlayGame" as const },
  { key: "order", icon: ShoppingCart, label: "Order out", handler: "onOrderOut" as const, badge: "30% off" },
] as const;
      ...
      {chips.map((chip, idx) => {
        ...
        return (
          <motion.button
            ...
            transition={{ type: "spring", stiffness: 300, damping: 20, delay: 0.28 + idx * 0.06 }}
            whileTap={{ scale: 0.92 }}
            className={cn(
