...
const categories = [
  { key: "vegetable" as const, label: "Veg", color: "bg-green-500", ring: "ring-green-500/30" },
  { key: "carb" as const, label: "Carb", color: "bg-amber-500", ring: "ring-amber-500/30" },
  { key: "protein" as const, label: "Protein", color: "bg-rose-500", ring: "ring-rose-500/30" },
];
...
export default function BalanceIndicator({ sides }: BalanceIndicatorProps) {
  ...
  const isBalanced =
    present.vegetable && present.carb && present.protein;

  const presentLabels = categories
    .filter((c) => present[c.key])
    .map((c) => c.label.toLowerCase());

  const tooltipText = presentLabels.length > 0
  ...
  return (
    <motion.div
      ...
      transition={prefersReduced ? { duration: 0.2, delay: 0.4 } : { duration: 0.3, delay: 0.8 }}
      title={tooltipText}
    >
