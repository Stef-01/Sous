...
## P0 — correctness (next loop)

- [ ] **Swipe planner manufactures dead meals** — `patternsToCandidates` slugifies pattern _names_; 0/16 match the catalog → planned meals have no recipe/image, "Cook now" dead-ends, never pin on Today. Re-source candidates from the real catalog (like `buildQuestDishes`) or map patterns→real `recipeSlug`s. `plan/page.tsx:52-68`, `swipe-pool.ts`.
- [ ] **Combined cook traps you on Back across dishes** — first step of dish 2+ → Back jumps to Grab (no `prevDish`). Add `prevDish()` to `use-cook-store.ts`; wire at `combined/page.tsx:465`.
- [ ] **Combined cook: sides logged but un-undoable** — 3-dish cook writes 3 diary entries; Undo removes only the main. Log one combined entry, or undo all session entries. `combined/page.tsx:396`, `win-screen.tsx:1060`.
...
- [ ] **Swipe planner empty on first run** — pool coverage-gated (no staples) → empty pantry = empty planner. Floor to N top cards regardless of coverage. `swipe-pool.ts:195`.
