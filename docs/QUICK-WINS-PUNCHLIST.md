...
| 2   | [src/app/cook/[slug]/page.tsx:357](../src/app/cook/[slug]/page.tsx) + [/cook/combined/page.tsx](../src/app/cook/combined/page.tsx) | "Steps coming soon" empty state when a dish lacks guided-cook data.                                                                         | **SHIPPED W51**       | Filtered unguided mains out of the default Today QuestCard pool and unguided side/drink/snack dishes out of the role-switched pool, so Today no longer anchors cooks to recipes without steps. |
| 3   | [src/components/content/reel-action-rail.tsx](../src/components/content/reel-action-rail.tsx) Share button                         | Calls `navigator.share()` only — silent fail when the Web Share API is unavailable (desktop Safari, older Firefox).                         | **SHIPPED W50**       | Added a tested Reels share payload/helper: native share when available, clipboard fallback with success/failure toast when unavailable.                                                        |
...
- **Item 3 (share fallback)** shipped in W50 — Reels now prefers native share and falls back to clipboard + toast when `navigator.share` is unavailable.
- **Item 2 (steps-coming-soon dead-end)** shipped in W51 — the Today QuestCard pools now exclude no-step mains, sides, drinks, and snacks before they reach the deck.
