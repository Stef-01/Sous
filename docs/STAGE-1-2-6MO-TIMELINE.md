...
## 0.5 Track classification — AUTO-BUILD vs FOUNDER-GATED (added 2026-05-01)

Per CLAUDE.md rule 12 (load-bearing): every week below is now classified.

| Week     | Title                                                                      | Track                                                                                     | Founder action needed (if gated)                                                                                                                                                                                                                |
| -------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W1       | Nutrition data ADR                                                         | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W2       | Nutrition schema + DRI/DV tables                                           | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W3       | Per-recipe nutrition (wave 1) + Stanford content wave 1                    | **MIXED**: USDA ingest is AUTO; Stanford swap is GATED on permission docs/images          | Provide Stanford-source URLs + permission screenshots in `docs/content-sources/<slug>/`                                                                                                                                                         |
| W4       | Safe phrasings + spotlight engine                                          | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W5       | Kid-friendliness labels (wave 1)                                           | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W6       | Kid-friendliness scorer + rebalancer                                       | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W7       | useParentMode hook + coach quiz extension                                  | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W8       | Spice / KidsAteIt / Exposure hooks                                         | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W9       | Profile/Settings sheet + KidFriendlyHint                                   | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W10      | Spice slider + ComponentSplit toggle                                       | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| W11      | KidSwap chip + AI fallback + Stanford wave 2                               | **MIXED**: chip is AUTO ✓ shipped; Stanford swap GATED                                    | Same as W3                                                                                                                                                                                                                                      |
| W12      | Win-screen KidsAteIt + Lunchbox + NutrientSpotlight + Content parent track | **AUTO** ✓ shipped                                                                        | —                                                                                                                                                                                                                                               |
| **W13**  | **Clerk auth enforcement**                                                 | **FOUNDER-GATED**                                                                         | Create Clerk app, add `CLERK_*` env vars to Vercel, share `.env.local`. Autonomous prep available now: Clerk middleware shell + protectedProcedure flip in tRPC router behind a `AUTH_REQUIRED` env flag.                                       |
| **W14**  | **Neon Postgres + Drizzle**                                                | **FOUNDER-GATED**                                                                         | Provision Neon DB, set `DATABASE_URL`. Autonomous prep available now: Drizzle schema additions for `parent_profile` / `kid_friendliness_label` / `kids_ate_it_log` + `pnpm db:push` script + a `useDbOrLocalStorage` write-through abstraction. |
| **W15**  | **Cloudflare R2**                                                          | **FOUNDER-GATED**                                                                         | Provision R2 bucket, set `R2_BASE_URL`. Autonomous prep available now: `imageSrc(slug)` helper that prefers `R2_BASE_URL` env then falls back to `/public/food_images/`. Migration script `scripts/content/upload-to-r2.ts`.                    |
| **W16**  | **Upstash Redis**                                                          | **FOUNDER-GATED**                                                                         | Provision Upstash, set `UPSTASH_REDIS_REST_URL` + token. Autonomous prep: rate-limit middleware shell that no-ops without the env var; per-endpoint cache wrapper with in-memory LRU fallback.                                                  |
| **W17**  | **Sentry + analytics + Stanford wave 3**                                   | **MIXED**: Sentry/analytics need DSN + project; Stanford swap GATED                       | Provide Sentry DSN + Vercel Analytics token + Stanford content. Autonomous prep: Sentry init shell + analytics-event taxonomy module that no-ops without env vars.                                                                              |
| W18      | Performance pass + Lighthouse                                              | **AUTO (mostly)**                                                                         | Lighthouse-CI gate needs deployed env; tree-shake + lazy-load + bundle audit are AUTO.                                                                                                                                                          |
| **W19**  | **Stanford content wave 4**                                                | **FOUNDER-GATED**                                                                         | See W3. Autonomous prep: image-save script ✓ shipped; content schema fields ✓ shipped. Real swap awaits permission docs.                                                                                                                        |
| **W19b** | **Content visual V2 redesign**                                             | **AUTO** ← active sub-phase                                                               | —                                                                                                                                                                                                                                               |
| W20      | Kid-friendliness label expansion (wave 2)                                  | **AUTO**                                                                                  | Extend `kid-friendliness-labels.ts` from 30 to ~75 meals.                                                                                                                                                                                       |
| **W21**  | **Closed beta cohort 1**                                                   | **FOUNDER-GATED**                                                                         | Recruit 10 households. Autonomous prep: in-app invite-flow + telemetry funnel scaffolded behind `BETA_OPEN` flag.                                                                                                                               |
| W22a     | Beta-fix + a11y + i18n + nav sweep + Stanford freshen                      | **MIXED**: a11y + i18n scaffold AUTO; beta-fix needs cohort-1 signal                      | Provide cohort-1 fix priorities.                                                                                                                                                                                                                |
| W22b     | Reels V2 ✓ + animation deep-dive                                           | **AUTO (Reels V2 ✓ shipped)** ← animation deep-dive next                                  | —                                                                                                                                                                                                                                               |
| **W23**  | **Legal review**                                                           | **FOUNDER-GATED**                                                                         | Engage food-advertising counsel ($5–15k). Autonomous prep: SAFE-phrasings build-time linter shell that fails the build if any string outside the locked list ships.                                                                             |
| **W24**  | **Closed beta cohort 2**                                                   | **FOUNDER-GATED**                                                                         | Recruit 50 households + 2–3 RDs.                                                                                                                                                                                                                |
| **W25**  | **Launch hardening**                                                       | **MIXED**: SLO/SLI definitions AUTO; landing page rewrite AUTO; pricing decision FOUNDER. | Confirm pricing posture (currently locked free per ADR 0001).                                                                                                                                                                                   |
| W26      | Buffer + retro                                                             | **AUTO**                                                                                  | Catch-up + retrospective doc.                                                                                                                                                                                                                   |

**Sequencing implication.** AUTO weeks I can keep shipping autonomously without any founder action: W18 (perf), W19b (Content V2 — active now), W20 (label expansion), W22b animation deep-dive, W26 (retro), PLUS all the autonomous prep work for the FOUNDER-GATED weeks (Clerk middleware shell, Drizzle schema, R2 src helper, Redis rate-limit shell, Sentry init shell, SAFE-phrasing linter). That's ~6 weeks of additional autonomous capacity available before any external action is required.

When the founder provides Clerk creds → W13 lands in 1–2 days because the shell is pre-built. Same pattern for Neon, R2, Redis, Sentry. The autonomous prep is what shrinks every founder-gated week from "1 work week" down to "1 config edit + smoke test."

**Why this section exists.** The original timeline scheduled W13–W17 + W19 as if they were normal AI-executable weeks. They are not — each requires an external account or third-party permission the AI does not have. CLAUDE.md rule 12 (load-bearing) was added the same day to prevent this from happening again. Every future planning doc must classify deliverables as AUTO or FOUNDER-GATED up front, with autonomous-prep callouts on the gated ones.
