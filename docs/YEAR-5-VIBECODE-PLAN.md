...
> **Filed:** 2026-11-29 · **Updated:** 2026-05-04 (Sprint D + E shipped; cursor at Y5 W20 → Sprint F next).
...
> **Current cursor (2026-05-04):** Sprints A through E are ✓ Shipped. Sprint **F (W21–W24, Annual recap)** is next-up by calendar order; Sprint **N (W41–W44, Cuisine Compass)** is being pulled forward into mixed-execution because the substrate is independent of F-I and the visible-polish payoff is high. See the per-sprint Status column for the canonical truth.
...
| **J**  | **W37-W40** | **Eat Out agentic mode**                      | **`/eat-out` route · Michelin-Guide dish-first card stack · venue-reveal swipe · agent search wrapper · stub-mode venue fixtures (8 dishes / 4 venues) · "Eat Out" entry in More-Options "Quick helpers" section.**                                                                                                                                                                                                                                                               | New                               |
| **N**  | **W41-W44** | **Cuisine Compass map game**                  | **GeoGuessr-grade daily dish-pinning game · MapLibre wrapper component (custom Sous-cream Field-Notes-pocket-atlas style) · drag-and-drop pin (React-Aria + arrow-key fallback) · 50-dish curated dataset (`src/data/compass-dishes.ts`) · daily-puzzle UI surface · reveal animation (great-circle line + score tick-up) · Wordle-share trigger + clipboard · "Cook this dish tonight" CTA on reveal · pod-vs-pod leaderboard surface. See `CUISINE-COMPASS-MAP-GAME-PLAN.md`.** | New                               |
...
- **Tests:** 2943 passing (190 test files); +16 new in the most recent ship (Eco summarize-savings, celebrated-challenges, dish-to-facets, active-challenge-banner helper).
- **Sprints A–E shipped.** Substrate + UI live across QuestCard / ResultStack / cook flow / Profile sheet / Today / `/path/eco` / Community.
...
- **Next-up by calendar:** Sprint **F** (Annual recap surface, W21–W24).
- **Pulled forward:** Sprint **N** (Cuisine Compass) substrate-first; UI deferred until MapLibre dependency lands.
- **Still to ship:** Sprints F (recap) · G (pod challenges 2.0) · H (retention analytics) · I (cohort dashboard) · J (Eat Out) · N (Compass) · K (perf audit) · L (intelligence polish) · M (year close).
