...
| **D**  | **W13-W16** | **Profile editor UI + Eco UI + Community polish** | **(expanded per `docs/CHAT-REQUEST-AUDIT-2026-05-04.md`)** · Profile sheet `Preferences` section (cuisine/flavor/dislike chips · "Where this came from" detail · suppress + reset flows) · time-of-day pattern row · `usePreferenceProfile` localStorage hook · `recordPreferenceSignal` wired into QuestCard / ResultStack / cook flow · Eco Mode toggle in Profile sheet · win-screen "+X kg saved" line · `/path/eco` dashboard · Today eco-progress chip · end-of-challenge celebration · active-challenge banner on Community · pod activity feed · demo-state reset affordance. | New            |
| **E**  | **W17-W20** | **Agentic recipe search + Unverified UI**         | **(expanded per audit doc)** · "Send agent" CTA on empty craving search · Anthropic+Tavily wrapper (stub V1 · real-mode flips on `ANTHROPIC_API_KEY`) · Unverified recipe card UI with provenance pill + tap-for-info · separate-shelf surfacing in search results.                                                                                                                                                                                                                                                                                                                   | New            |
