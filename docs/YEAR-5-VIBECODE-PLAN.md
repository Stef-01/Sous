...
> **Filed:** 2026-11-29 · **Updated:** 2026-05-04 (Sprints A–G + J + N shipped; cursor at Y5 W28+).
...
> **Current cursor (2026-05-04):** Sprints A–G + J + N are ✓ Shipped. Remaining: Sprint **H** (Retention analytics, W29–W32) → **I** (Cohort dashboard, W33–W36) → **K** (Perf audit, W45–W48) → **L** (Intelligence-layer real-mode polish, W49–W50) → **M** (Year close, W51–W52). See the per-sprint Status column for the canonical truth.

> **Cross-cutting hardening:** `<DeadEndShell>` (P0 RCA 2026-05-04) is the load-bearing safety net for every error / not-found terminal state — fixed top-left back button + `min-h-dvh` + safe-area padding + hard `<Link>` anchor fallback. Project-wide invariant test (`dead-end-shell.test.ts`) walks `src/app` and fails the build if any `error.tsx` / `not-found.tsx` skips the shell.
...
| G      | W25-W28 | Pod challenges 2.0                            | `lib/pod/weekly-themes.ts` — 6-theme rotation deterministic per ISO week · `lib/pod/cohort-pod-matcher.ts` — engagement-tier + acquisition-week ranker · `lib/pod/cross-pod-leaderboard.ts` — weekly mean-per-member + rolling-7d ranker · `<WeeklyThemeBanner />` on Community · `/community/leaderboard` with own-pod call-out + stub peer pods (38 substrate tests).                                                                               | ✓ Shipped (Sprint G)              |
