...
- **E3 — Entrance system. ✅ SHIPPED 2026-06-17 (`15480b9`).** New `premiumEntrance(reducedMotion)` framer helper mirrors `--entrance-premium` (fade + 6px rise + 2px blur clearing on `--ease-out`/`--dur-slow`), collapsing to a plain instant fade under reduced motion. Routed the Today hero (craving bar + meal-queue preview) through it. _Remaining:_ extend to the bottom sheets/modals + card reveals on Path/Content as those screens are swept (focal only, never lists).
- **E4 — Tactile + state pass. ✅ SHIPPED 2026-06-17 (`5ef80c5`).** `TAP_SCALE` (0.98) / `TAP_SCALE_SM` (0.94) tokens; the `Pressable`/`TapFeedback`/`ChipFeedback` primitives route through them, and `Pressable` ships a real disabled state. Glossify tooltip now lifts + blurs in (house ease, hairline-ring shadow), reduced-motion gated. Shipped `docs/STATE-COVERAGE.md` (the idle→success matrix + the shared tactile standard). _Remaining (called out in the doc):_ a built-in `loading` state on the button primitives + a shared inline error treatment, and extending the matrix to Path/Content components.
