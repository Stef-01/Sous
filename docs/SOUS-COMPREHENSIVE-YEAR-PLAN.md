...
- **E2 — Shadow/hairline sweep.** Migrate focal surfaces (cards, sheets, modals, hero, header, result stack) from `border`→hairline-ring stack; hover animates the stack (movement/opacity only). Dividers stay cheap.
  - **Batch 1 ✅ SHIPPED 2026-06-17 (`e5c650a`) — Today + Nutrition + the `Card` primitive.** Craving bar (lifts on hover), meal-preview/empty/caught-up cards, and the calories/macros/key-nutrients cards now ride `--shadow-card` (ring + soft layers), no border. Confirmed the result-stack needs none (its bordered bits are chips/buttons). Chips, toggles, inputs, dashed empty-states, brand-tinted accent cards, and dense list rows keep cheap borders by design.
  - **Batch 2 ✅ SHIPPED 2026-06-17 (`076d05b`) — E2 COMPLETE.** Path (weekly-goal / journey-summary / plan-balance cards + the achievements modal), Content magazine cards + Community sections (lift on hover), the Profile + AI-import sheets (heavy `shadow-2xl`→ringed `--shadow-raised`), and the Today header (`border-b`→`--shadow-header`). Kept by design: chips, inputs, dashed empty-states, brand-tinted (green/amber/gold) accent cards, the scrapbook paper card, gradient modals, and dense list dividers. Verified Today/Content/Path in-browser; 17 files, lint + build + 4011 tests green.
- **E3 — Entrance system. ✅ SHIPPED 2026-06-17 (`15480b9`).** New `premiumEntrance(reducedMotion)` framer helper mirrors `--entrance-premium` (fade + 6px rise + 2px blur clearing on `--ease-out`/`--dur-slow`), collapsing to a plain instant fade under reduced motion. Routed the Today hero (craving bar + meal-queue preview) through it. **Threads closed (`96ef6f2`):** the PlanAddSheet + week-plan manage sheet now slide up via the shared `sheetMotion` contract + backdrop fade, wrapped in AnimatePresence (clean enter+exit) — verified in-browser.
- **E4 — Tactile + state pass. ✅ SHIPPED 2026-06-17 (`5ef80c5`).** `TAP_SCALE` (0.98) / `TAP_SCALE_SM` (0.94) tokens; the `Pressable`/`TapFeedback`/`ChipFeedback` primitives route through them, and `Pressable` ships a real disabled state. Glossify tooltip now lifts + blurs in (house ease, hairline-ring shadow), reduced-motion gated. Shipped `docs/STATE-COVERAGE.md` (the idle→success matrix + the shared tactile standard). **Threads closed (`96ef6f2`):** `Pressable`/`TapFeedback` gained a real `loading` state (spinner + auto-disable + aria-busy) and a shared `InlineError` (role=alert) landed for field/action failures. _Optional follow-up:_ adopt `loading`/`InlineError` on the actual submit forms (pod create/join, AI-import) — per-call-site wiring; the primitives are ready.
- **E5 — Physics inputs. ✅ SHIPPED 2026-06-17 (`b3f95ee`).** New `PhysicsSlider` (thumb springs to magnetic integer stops + catch ripple + haptic), with full slider a11y (role=slider, aria-value\*, ←/↓/→/↑/Home/End) and a reduced-motion fallback. Replaced the Pantry-Mode tolerance native range. Momentum/rubber-band intentionally omitted for a short discrete slider (crisp snaps > overshoot). Pure `slider-math` + 6 tests; verified in-browser. _Remaining:_ adopt it for serving-size + the other 3 native ranges (one-line swaps).
- **E6 — Typography + color + spacing. ✅ SHIPPED 2026-06-17 (`9bec9fb`) — light theme complete.** (a) **Type:** added `--tracking-display`/`--tracking-title`, applied negative tracking to `.sous-display`/`.sous-title`/`.sous-h2` (the fastest premium jump after shadows; verified `.sous-title` = −0.42px). (b) **Spacing:** audited — the app already rides Tailwind's 4/8pt scale; the only arbitrary px are intentional offsets (a 2px hairline inset, header/safe-area heights), so no rework needed. (c) **Color depth:** the E1 re-tune already gives dark elevation a white hairline-ring "lighter surface." _Deferred (not reachable):_ deeper dark layered-surface work — `[data-theme="dark"]` is defined but NOT wired (no toggle anywhere), so polishing it is invisible; it belongs with a future dark-mode-toggle feature. Gradient use is already restrained (mascots/badges/accent tints only).
- **E7 — 60fps + reduced-motion regression sweep. ✅ SHIPPED 2026-06-17 (`d331599`).** New `reduced-motion-contract.test`: every E-track helper collapses to instant/flat under reduced motion, and animating `filter:blur` is allowlisted to focal surfaces only (no blur on lists — the 60fps killer, rule 9). Unit-level floor — `tests/e2e/` is empty, so a Playwright reduced-motion run is the future layer on top; the `reduced-motion-gate` eslint rule already forces the gate into every framer file.
