...
- **E2 — Shadow/hairline sweep.** Migrate focal surfaces (cards, sheets, modals, hero, header, result stack) from `border`→hairline-ring stack; hover animates the stack (movement/opacity only). Dividers stay cheap.
  ...
  - **Remaining batches:** Path screens (skill tree, planner week/manage + `PlanAddSheet`, pantry, shopping list), Content magazine cards, the bottom sheets/modals' inner focal cards, and the app header. Same rule: focal raised surfaces → ring stack; chips/dividers stay cheap. Each batch eyeballed on its screen.
...
- **E6 — Typography + color + spacing. ◐ TYPE SHIPPED 2026-06-17 (`9bec9fb`).** Added `--tracking-display`/`--tracking-title` + applied negative tracking to `.sous-display`/`.sous-title`/`.sous-h2` — the fastest premium jump after shadows; verified (`.sous-title` computes −0.42px). _Remaining (broad audits):_ color depth (dark layered surfaces beyond the E1 white ring), an 8pt spacing pass, gradient/tint restraint.
